...
TEST_ON_VIDEO = True
VIDEO_SOURCE = "data/06-25-2022 18-47-40/out.mp4"
...
class ColorDetection:
    ...
    def save(self, path: str = None):
        path = path if path else "config/color_detection.json"
    ...
    def load(self, path: str = None):
        ...
        if os.path.exists(path):
            ...
            raise Exception(
                f"Config path {path} for color detection not exists")
    ...
    def get_kmeans_thresh(self, img: np.ndarray, k: int = 1, show=False) -> np.ndarray:
        ...
        colors = kmeans[(center[0] - range):(center[0] + range),
                        (center[1] - range):(center[1] + range)].reshape(-1, 3)
...
class ColorDetectionUI:
    ...
    def create_trackbars(self):
        cv.createTrackbar(
            self.trackbar_names["L_min"], self.wnd, 0, 255, self.create_trackbar_callback("L_min"))
        cv.createTrackbar(
            self.trackbar_names["L_max"], self.wnd, 0, 255, self.create_trackbar_callback("L_max"))
        cv.createTrackbar(
            self.trackbar_names["a_min"], self.wnd, 0, 255, self.create_trackbar_callback("a_min"))
        cv.createTrackbar(
            self.trackbar_names["a_max"], self.wnd, 0, 255, self.create_trackbar_callback("a_max"))
        cv.createTrackbar(
            self.trackbar_names["b_min"], self.wnd, 0, 255, self.create_trackbar_callback("b_min"))
        cv.createTrackbar(
            self.trackbar_names["b_max"], self.wnd, 0, 255, self.create_trackbar_callback("b_max"))
    ...
    def update_trackbars_pos(self):
        cv.setTrackbarPos(
            self.trackbar_names["L_min"], self.wnd, self.color_detection.min[self.color_index["L"]])
        cv.setTrackbarPos(
            self.trackbar_names["L_max"], self.wnd, self.color_detection.max[self.color_index["L"]])
        cv.setTrackbarPos(
            self.trackbar_names["a_min"], self.wnd, self.color_detection.min[self.color_index["a"]])
        cv.setTrackbarPos(
            self.trackbar_names["a_max"], self.wnd, self.color_detection.max[self.color_index["a"]])
        cv.setTrackbarPos(
            self.trackbar_names["b_min"], self.wnd, self.color_detection.min[self.color_index["b"]])
        cv.setTrackbarPos(
            self.trackbar_names["b_max"], self.wnd, self.color_detection.max[self.color_index["b"]])
    ...
    def update_cv_gradient_masked(self):
        if self.clicked_button == 'common':
            ...
            for color_comb in combinations(self.color_detection.colors, 2):
                ...
                color_mask1 = self.color_detection.get_mask(self.cv_gradient, cv.COLOR_BGR2LAB)
                self.color_detection.set_color(color_comb[1])
                color_mask2 = self.color_detection.get_mask(self.cv_gradient, cv.COLOR_BGR2LAB)
            ...
            self.cv_gradient_masked = self.color_detection.apply_mask(
                self.cv_gradient)
    ...
    def update_cv_test_image_masked(self):
        if self.clicked_button == 'common':
            ...
            for color in self.color_detection.colors:
                self.color_detection.set_color(color)
                color_mask = self.color_detection.get_mask(self.cv_test_image, cv.COLOR_BGR2LAB)
...
if __name__ == "__main__":
    ...
    img = cv.imread("utils/figure_yellow.png")
    thresh = color_detection.get_kmeans_thresh(img)
    cv.imshow("Thresh", thresh)
    color = color_detection.get_color(img, thresh)
